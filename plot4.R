...
par(mfrow = c(2, 2))
with(targetData, {
  plot(targetData$DateTime, targetData$Global_active_power, ylab="Global Active Power",xlab="",type="l")
  plot(targetData$DateTime, targetData$Voltage, xlab="datetime", ylab = "Voltage", type="l")
  plot(targetData$DateTime,targetData$Sub_metering_1,type="n", xlab="", ylab="Energy Sub Metering", col="black",ylim=c(0,30))
    ...
    legend("topright", lty=c(1,1,1), bty="n",col = c("black","red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"),cex=0.6)
  plot(targetData$DateTime, targetData$Global_reactive_power,xlab="datetime",ylab="Global_reactive_power",type="l")
})
